...
if __name__ == '__main__':
    sc = SparkContext()  # note: our connection to the cluster
    # SparkConf()  # note: our interface with that connection from SparkContext
    ...
    #########################################################################
    # note: RDDs are hard to work with directly, so in this course you'll be
    # using the Spark DataFrame abstraction built on top of RDDs.

    # Create spark session
    spark = SparkSession.builder.getOrCreate()
    # spark = SparkSession()
    # note: use SparkSession.builder.getOrCreate() instead of SparkSession()
    # directly in case a session already exists
    ...
    # Don't change this query
    query = ("SELECT origin, dest, COUNT(*) as N "
    ...
    model_data = model_data.filter(
        "arr_delay is not NULL and dep_delay is not NULL and "
    ...
    vec_assembler = VectorAssembler(
        inputCols=["month", "air_time", "carrier_fact", "dest_fact",
                   "plane_age"], outputCol='features')

    # Make the pipeline
    flights_pipe = Pipeline(
        stages=[dest_indexer, dest_encoder, carr_indexer, carr_encoder,
                vec_assembler])
    ...
    # # Call lr.fit()
    # best_lr = lr.fit(dataset=training)  # note: for fitting with no cv
